...
- **Blank New:** `urls.current.new` in `state.svelte.ts` now serializes `{ ...defaultState, code: '' }` so "New" opens a blank canvas instead of the Christmas flowchart demo.
- **AI via Vercel Gateway (BYOK):** User pastes Vercel AI Gateway key in new `AiSettings.svelte` card (stored in `localStorage` as `mermaid-ai-gateway-key` / `mermaid-ai-model`, default `openai/gpt-4o-mini`). Client-side calls via `ai` + `@ai-sdk/gateway` (`generateText` with `createGateway({ apiKey })`). Gutter AI prompt (`DesktopEditor` + `AIPromptPopup`) and syntax-error "AI Repair" (`Editor.svelte`) now hit the Gateway directly and patch the editor via `updateCode`; `AiSettings` also offers Generate/Edit buttons. Previous mermaid.ai links (McWrapper + `MERMAID_IS_ENABLED_MERMAID_CHART_LINKS`) remain disabled.
